...
#
import pygtk, gtk, gobject, glob, os, time, sys
...
class mk_gui:
    ...
    def syn_button_action(self, widget, action):
        ...
        if action=='Start Synthesis':
            ...
            # if the synthesis process exists already and hasn't terminated exit
    ...
    def check_for_new_ver(self, widget):
        ...
        _answer = new_version.check_for_new_version()[1]
        print _answer
    ...
    def __init__(self):
        ...
        # set application icon which must me together with the 
	exe_path = os.path.dirname(sys.argv[0])
        try:
            self.window.set_icon_from_file(os.path.join(exe_path, "boot-icon.svg"))
        except:
            try:
                self.window.set_icon_from_file(os.path.join(exe_path, "boot-icon.png"))
            except:
                print "Warning: not application icon found."
            
        #self.window.connect("delete_event", self.delete)
        self.window.connect("destroy", self.destroy_progress)
        self.window.set_border_width(2)
        self.window.set_size_request(1020, 500)
        import version
